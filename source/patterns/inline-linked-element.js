...
/**
    @name inline-linked-element
    @type inline
    @outputs
      - `href`: link URL
      - `isref`: is link data from a ref?
      - `title`: link title
      - `value`: link text

    @description
      Based on CommonMark *links* and *images*.
      <https://spec.commonmark.org/0.31.2/#links>
      <https://spec.commonmark.org/0.31.2/#images>

    @param { object } data
      - `image`: `true` if image link
*/
export default function ({ image })
{
    ...
    let pattern =
    {
        ...
        {
            let { isref, url, title } = state, data = { chunks: content };
